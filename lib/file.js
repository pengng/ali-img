...
class AliImg {
  ...
  putObject (fullpath, objectName, callback) {
    this.putObjectFromStream(fs.createReadStream(fullpath), objectName, callback)
  }

  wrapOnResponse(callback) {
    const buffers = []
    return function (res) {
      if (!(res.statusCode >= 200 && res.statusCode < 300)) {
        res.on('data', buffers.push.bind(buffers)).on('end', function () {
          callback(new HTTPError(statusCode, Buffer.concat(buffers).toString()))
        })
      } else {
        res.on('end', callback).resume()
      }
    }
  }

  putObjectFromStream(readable, objectName, callback) {
    let writable = null
    let headers = null
    const onData = function (chunk) {
      if (!writable) {
        headers = {
          'Content-Type': fileType(chunk).mime
        }
        writable = this.preRequest('put', objectName, headers).on('error', callback).on('response', this.wrapOnResponse(callback))
      }
    ...
    readable.on('error', callback).on('data', onData.bind(this)).on('end', onEnd.bind(this))
  }
  ...
  putObjectFromUrl (url, objectName, callback) {
    ...
    const onResponse = function (res) {
      res.pipe(
        this.preRequest('put', objectName, res.headers).on('error', callback).on('response', this.wrapOnResponse(callback))
      )
    }
  ...
  save (objectName, callback) {
    ...
    this.stream().pipe(
      this.client.preRequest('put', objectName, headers).on('error', callback).on('response', this.client.wrapOnResponse(newCallback.bind(this)))
    )
  }
  ...
  putObjects (callback) {
    const iteration = function (item, next) {
      if (item.path instanceof Readable) {
        this.client.putObjectFromStream(item.path, item.objectName, next)
